    ...
    Box::leak(
        nodes
            ...
            .map(|node| {
                ...
                if node_type.eq(env.intern("template-note:element").unwrap()) {
                ...
                } else if node_type.eq(env.intern("template-note:dynamic").unwrap()) {
                    let id: usize = plist_get(node_fields, ":id");
                    dioxus_core::TemplateNode::Dynamic { id }
                } else if node_type.eq(env.intern("template-note:text").unwrap()) {
                    ...
                    panic!(
                        "Unknown node type: {:?}. Expected one of: 'template-note:element', 'template-note:dynamic', or 'template-note:text'",
                        node_type
                    );
                }
            })
